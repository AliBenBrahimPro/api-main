...
router.get('/mostDominantCategoriesByOrder', async (req, res) => {
  try {
    const mostDominantCategories = await Order.aggregate([
      ...
      { $unwind: "$productInfo" },
      { $group: { _id: "$productInfo.categories", totalQuantity: { $sum: "$products.quantity" } } },
      { $sort: { totalQuantity: -1 } },
      { $limit: 3 },
      { $project: { category: "$_id", totalQuantity: 1 } }
    ]);
