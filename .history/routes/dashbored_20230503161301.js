...
export default function Chart({ title }) {
  ...
  useEffect(() => {
    // Fetch userTrafficCount data from the backend API
    const fetchUserTrafficCount = async () => {
      try {
        const response = await axios.get("/usertrafficcount");
      ...
      <ResponsiveContainer width="100%" aspect={4 / 1}>
        <LineChart data={data}>
          <XAxis
            ...
            tickMargin={10}
            axisLine={false}
            tickFormatter={(value) => value.slice(0, 3)} // Format month label to show only the first 3 letters
          />
          <Line type="monotone" dataKey="count" stroke="#4EE1D4" />
