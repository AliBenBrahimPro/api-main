...
// GET 3 most Dominant Categories  according products


router.get('/mostDominantCategories', async (req, res) => {
  try {
    const mostDominantCategories = await Product.aggregate([
      {
        $group: {
          _id: "$categories",
          count: { $sum: 1 }
...
// GET user traffic count per month
router.get('/usertrafficcount', async (req, res) => {
  try {
    const userTrafficCount = await User.aggregate([
      {
        $group: {
          _id: { $month: "$createdAt" },
          NB_user: { $sum: 1 }
    ...
    const monthLabels = userTrafficCount.map((item) => {
      const monthNumber = item._id;
      const monthName = new Date(Date.UTC(2021, monthNumber - 1, 1))
        .toLocaleString('en-US', { month: 'long' });
      return { month: monthName };
    });

    const userTrafficCountWithLabels = userTrafficCount.map((item, index) => {
      return { ...item, ...monthLabels[index] };
    });

    res.status(200).json(userTrafficCountWithLabels);
  } catch (err) {
    res.status(500).json(err);
  }
});
