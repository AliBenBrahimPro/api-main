...
router.get('/mostorderedcategories', async (req, res) => {
  try {
    const mostOrderedCategories = await Order.aggregate([
      ...
      { $limit: 4 },
      {
        $group: {
          _id: null,
          categories: { $push: { category: "$category", totalQuantity: "$totalQuantity" } }
        }
      },
      { $project: { _id: 0 } }
    ]);

    res.status(200).json(mostOrderedCategories[0].categories);
  } catch (err) {
    res.status(500).json(err);
  }
});
